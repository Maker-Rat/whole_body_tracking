...
def quat_to_rot_matrix(quat_wxyz, out=None):
...
def get_robot_state(data, model, cfg, state_cache=None, torso_body_id=None, rot_mat=None, isaac_to_mujoco=None):
    ...
    if isaac_to_mujoco is None:
        isaac_to_mujoco = cfg.ISAAC_TO_MUJOCO
    
    # Base pose (first 7 elements of qpos: x, y, z, qw, qx, qy, qz)
    base_pos = state_cache['base_pos']
    base_quat_wxyz = state_cache['base_quat']
    np.copyto(base_pos, data.qpos[:3])
    np.copyto(base_quat_wxyz, data.qpos[3:7])
    ...
    if rot_mat is None:
        ...
        quat_to_rot_matrix(base_quat_wxyz, out=rot_mat)
    np.dot(rot_mat.T, base_lin_vel, out=state_cache['base_lin_vel'])
    np.dot(rot_mat.T, base_ang_vel, out=state_cache['base_ang_vel'])
    
    # Joint positions and velocities (after the 7 base DOFs) - in MuJoCo order
    joint_pos_mujoco = data.qpos[7:7+cfg.num_actions]
    joint_vel_mujoco = data.qvel[6:6+cfg.num_actions]
    
    # Convert from MuJoCo order to Isaac Lab order
    # mujoco_data[ISAAC_TO_MUJOCO] reorders: result[isaac_idx] = mujoco_data[mujoco_idx]
    np.take(joint_pos_mujoco, isaac_to_mujoco, out=state_cache['joint_pos'])
    np.take(joint_vel_mujoco, isaac_to_mujoco, out=state_cache['joint_vel'])
    ...
    if torso_body_id is None:
        torso_body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "link_torso_yaw")
    if torso_body_id >= 0:
        np.copyto(state_cache['anchor_pos'], data.xpos[torso_body_id])
        np.copyto(state_cache['anchor_quat'], data.xquat[torso_body_id])
    else:
        # Fallback to base if torso not found
...
def build_observation(robot_state, motion, time_step, last_action, cfg, obs_cache=None):
    """
    ...
    obs_buf = obs_cache['obs_buf']
    joint_pos_rel = obs_cache['joint_pos_rel']
    default_pos = obs_cache['default_pos']
    ...
    idx += cfg.num_actions
    obs_buf[idx:idx+cfg.num_actions] = robot_state['joint_vel']
    idx += cfg.num_actions
    obs_buf[idx:idx+cfg.num_actions] = last_action
    
    return obs_buf


def build_pd_cache(cfg, default_pos_isaac, mujoco_to_isaac):
    """Precompute PD gains and buffers for fast control."""
    ...
    return {
        'default_pos': default_pos_isaac,
        'kp': kp,
        'kd': kd,
        'tau_limit': tau_limit,
        'target': np.zeros(cfg.num_actions),
        'tau_isaac': np.zeros(cfg.num_actions),
        'kd_term': np.zeros(cfg.num_actions),
        'tau_mujoco': np.zeros(cfg.num_actions),
        'mujoco_to_isaac': mujoco_to_isaac,
    }


def pd_control(target_pos_isaac, current_pos_isaac, current_vel_isaac, cfg, pd_cache=None):
    """
    ...
    default_pos = pd_cache['default_pos']
    kp = pd_cache['kp']
    kd = pd_cache['kd']
    tau_limit = pd_cache['tau_limit']
    target = pd_cache['target']
    tau_isaac = pd_cache['tau_isaac']
    kd_term = pd_cache['kd_term']
    
    # Target is action * scale + default (all in Isaac Lab order)
    np.add(target_pos_isaac, default_pos, out=target)
    
    # PD control in Isaac Lab order
    np.subtract(target, current_pos_isaac, out=tau_isaac)
    tau_isaac *= kp
    np.multiply(kd, current_vel_isaac, out=kd_term)
    tau_isaac -= kd_term
    
    # Clamp torques
    np.clip(tau_isaac, -tau_limit, tau_limit, out=tau_isaac)
    
    # Convert torques from Isaac Lab order to MuJoCo order
    # isaac_data[MUJOCO_TO_ISAAC] reorders: result[mujoco_idx] = isaac_data[isaac_idx]
    np.take(tau_isaac, pd_cache['mujoco_to_isaac'], out=pd_cache['tau_mujoco'])
    
    return pd_cache['tau_mujoco']
...
def load_policy(policy_path=None, onnx_path=None, wandb_path=None):
    ...
    if onnx_path is not None:
        ...
        time_step_np = np.zeros((1, 1), dtype=np.float32) if needs_time_step else None
        
        def policy_fn(obs, time_step=0):
            obs_np = obs.reshape(1, -1).astype(np.float32, copy=False)
            if needs_time_step:
                time_step_np[0, 0] = time_step
...
def find_mujoco_model():
    ...
    possible_paths = [
        # GMR assets folder
        "/media/marmot/606de469-2f76-4155-82bc-e2e657636ad7/Ritwik/GMR/assets/engineai_pm01_mod/pm_v2.xml",
        # Relative paths
        "../GMR/assets/engineai_pm01_mod/pm_v2.xml",
        "../../GMR/assets/engineai_pm01_mod/pm_v2.xml",
    ]
...
def run_sim2sim(policy_fn, motion, cfg, args):
    ...
    isaac_to_mujoco = np.asarray(cfg.ISAAC_TO_MUJOCO)
    mujoco_to_isaac = np.asarray(cfg.MUJOCO_TO_ISAAC)
    torso_body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "link_torso_yaw")
    ...
    default_pos_mujoco = default_pos_isaac[mujoco_to_isaac]
    ...
    state_cache = {
        ...
        'joint_pos': np.zeros(cfg.num_actions),
    ...
    pd_cache = build_pd_cache(cfg, default_pos_isaac, mujoco_to_isaac)
    current_pos_isaac = np.zeros(cfg.num_actions)
    current_vel_isaac = np.zeros(cfg.num_actions)
    target_pos = np.zeros(cfg.num_actions)
    ...
    try:
        while step_count * cfg.dt < cfg.sim_duration:
            ...
            if step_count % cfg.decimation == 0:
                ...
                # Build observation
                obs = build_observation(robot_state, motion, motion_step, last_action, cfg, obs_cache=obs_cache)
                ...
                # Scale action
                np.multiply(action, action_scales, out=target_pos)
                np.copyto(last_action, action)
            ...
            # mujoco_data[ISAAC_TO_MUJOCO] reorders: result[isaac_idx] = mujoco_data[mujoco_idx]
            np.take(current_pos_mujoco, isaac_to_mujoco, out=current_pos_isaac)
            np.take(current_vel_mujoco, isaac_to_mujoco, out=current_vel_isaac)
            # pd_control returns torques in MuJoCo order
            tau = pd_control(target_pos, current_pos_isaac, current_vel_isaac, cfg, pd_cache=pd_cache)
